...
class fvvdp:
    def __init__(self, display_name="standard_4k", display_photometry=None, display_geometry=None, color_space="sRGB",
                 foveated=False, heatmap=None, quiet=False, device=None, temp_padding="replicate",
    ...
    def load_config(self):
        ...
        self.sensitivity_correction = parameters[
            'sensitivity_correction']  # Correct CSF values in dB. Negative values make the metric less sensitive.
    ...
    def predict(self, test_cont, reference_cont, dim_order="BCFHW", frames_per_second=0, fixation_point=None):
        ...
        test_vs = fvvdp_video_source_array(test_cont, reference_cont, frames_per_second, dim_order=dim_order,
    ...
    def predict_video_source(self, vid_source, fixation_point=None):
        ...
        if self.do_heatmap:
            ...
            heatmap = torch.zeros([1, dmap_channels, N_frames, height, width], dtype=torch.float16,
                                  device=torch.device('cpu'))  # Store heatmap in the CPU memory
        else:
            heatmap = None
        ...
        for ff in range(N_frames):
            ...
            if is_image:
                ...
                if ff == 0:  # First frame
                    if self.temp_padding == "replicate":
                        ...
                        sw_buf[1] = vid_source.get_reference_frame(0, device=self.device).expand(
                            [1, 1, fl, height, width])
            ...
            if self.use_checkpoints:
                # Used for training
                Q_per_ch_block = checkpoint.checkpoint(self.process_block_of_frames, ff, R, vid_sz, temp_ch,
                                                       fixation_point, heatmap, use_reentrant=False)
            else:
    ...
    def do_pooling_and_jods(self, Q_per_ch, rho_band):
        ...
        if Q_per_ch.shape[1] == 2:  # If video
            per_tband_w = torch.stack(
                (torch.ones(1, device=self.device), torch.as_tensor(self.w_transient, device=self.device)[None]),
                dim=1)[:, :, None]
        else:  # If image
            per_tband_w = 1
        ...
        Q_jod = sign(self.jod_a) * ((abs(self.jod_a) ** (
                    1.0 / beta_jod)) * Q) ** beta_jod + 10.0  # This one can help with very large numbers
    ...
    def process_block_of_frames(self, ff, R, vid_sz, temp_ch, fixation_point, heatmap):
        ...
        if self.debug: self.tb.verify_against_matlab(R.permute(0, 2, 3, 4, 1), 'Rdata', self.device,
                                                     file='R_%d' % (ff + 1), tolerance=0.01)
        ...
        if self.do_heatmap:
            Dmap_pyr_bands, Dmap_pyr_gbands = self.heatmap_pyr.decompose(
                torch.zeros([1, 1, height, width], dtype=torch.float, device=self.device))
            ...
            for bb in range(self.lpyr.get_band_count() - 1):
                ...
                if self.local_adapt == "gpyr":
                    ...
                    L_bkg, R_f, T_f = self.compute_local_contrast(R_f, T_f,
                                                                  self.lpyr.get_gband(B_gbands, bb + 1)[1:2, ...],
                                                                  L_adapt)
                ...
                if self.debug: self.tb.verify_against_matlab(T_f, 'T_f_data', self.device,
                                                             file='T_f_%d_%d_%d' % (ff + 1, bb + 1, cc + 1),
                                                             tolerance=0.001)
                if self.debug: self.tb.verify_against_matlab(R_f, 'R_f_data', self.device,
                                                             file='R_f_%d_%d_%d' % (ff + 1, bb + 1, cc + 1),
                                                             tolerance=0.001)
                ...
                if N_nCSF[bb][cc] is None:
                    ...
                    if self.foveated:  # Fixation, parafoveal sensitivity
                        ...
                        ecc = self.display_geometry.pix2eccentricity(torch.tensor((vid_sz[1], vid_sz[0])), xx, yy,
                                                                     current_fixation_point + 0.5)
                    ...
                    S = self.cached_sensitivity(rho, self.omega[cc], L_bkg, ecc, self.csf_sigma) * 10.0 ** (
                                self.sensitivity_correction / 20.0)
                    ...
                    if self.contrast == "log":
                        N_nCSF[bb][cc] = self.weber2log(torch.min(1. / S, self.torch_scalar(0.9999999)))
                    else:
                        N_nCSF[bb][cc] = torch.reciprocal(S)
                ...
                if self.debug: self.tb.verify_against_matlab(D, 'D_data', self.device,
                                                             file='D_%d_%d_%d' % (ff + 1, bb + 1, cc + 1),
                                                             tolerance=0.1, relative=True, verbose=False)

                if self.do_heatmap:
                    if cc == 0:
                        self.heatmap_pyr.set_band(Dmap_pyr_bands, bb, D)
                    else:
                        w_temp_ch = [1.0, self.w_transient]
                        self.heatmap_pyr.set_band(Dmap_pyr_bands, bb,
                                                  self.heatmap_pyr.get_band(Dmap_pyr_bands, bb) + w_temp_ch[cc] * D)
        ...
        if self.do_heatmap:
            ...
            if self.heatmap == "raw":
                ...
                ref_frame = R[:, 0, :, :, :]
                heatmap[:, :, ff, ...] = visualize_diff_map(dmap, context_image=ref_frame,
                                                            colormap_type=self.heatmap).detach().type(
                    torch.float16).cpu()
    ...
    def get_info_string(self):
        ...
        return '"FovVideoVDP v{}, {:.4g} [pix/deg], Lpeak={:.5g}, Lblack={:.4g} [cd/m^2], {}{}"'.format(self.version,
                                                                                                        self.pix_per_deg,
                                                                                                        self.display_photometry.get_peak_luminance(),
                                                                                                        self.display_photometry.get_black_level(),
                                                                                                        fv_mode,
                                                                                                        standard_str)
